<script lang="ts">
	...
	const fontLoader = useLoader(FontLoader);
	const dm = fontLoader.load('dm.json');
	const mplus = fontLoader.load('mplus.json');
...
>
	{#each dataPointTitles as text, i}
		<T.Mesh position={[left, top - (textOffset + i * valueOffset), 0]}>
			<Text3DGeometry {text} size={titleSize} depth={0.01} font={$mplus} />
			<T.MeshStandardMaterial color={'black'} />
		</T.Mesh>
	{/each}
	{#each texts as textData}
		<T.Mesh position={[left, top - textData.topOffset, 0]}>
			<Text3DGeometry
				text={textData.text}
				size={textData.size}
				depth={0.01}
